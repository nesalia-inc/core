---
title: Try
description: Wrap synchronous functions that might throw exceptions
---

# Try

The `Try` type wraps synchronous functions that might throw exceptions. It captures the result or the error without using try-catch blocks.

## Overview

```typescript
type Try<T, E = Error> = TrySuccess<T> | TryFailure<E>;
```

- **`TrySuccess<T>`** - Represents a successful execution with value `T`
- **`TryFailure<E>`** - Represents a failure with error `E`

## Creating a Try

### attempt - Wrap a Function

```typescript
import { attempt, isOk, isErr } from "@deessejs/core";

const result = attempt(() => {
  return 42;
});

console.log(isOk(result)); // true
console.log(result.value); // 42
```

### attempt - Capture Exceptions

```typescript
import { attempt } from "@deessejs/core";

const result = attempt(() => {
  throw new Error("Something went wrong");
});

console.log(isErr(result)); // true
console.log(result.error.message); // "Something went wrong"
```

### attemptAsync - Wrap Async Functions

```typescript
import { attemptAsync } from "@deessejs/core";

const result = await attemptAsync(async () => {
  const response = await fetch("/api/data");
  return response.json();
});
```

## Transforming Try

### map - Transform the Value

```typescript
import { attempt, map } from "@deessejs/core";

const result = attempt(() => 10)
  .map(x => x * 2)
  .map(x => x + 1);

console.log(result.value); // 21
```

### flatMap - Chain Operations

```typescript
import { attempt, flatMap } from "@deessejs/core";

const parseNumber = (s: string): Try<number> => attempt(() => parseInt(s, 10));

const result = attempt(() => "42")
  .flatMap(parseNumber);
```

## Handling Try

### getOrElse - Get Value or Default

```typescript
import { attempt, getOrElse } from "@deessejs/core";

attempt(() => 10).getOrElse(0); // 10
attempt(() => { throw new Error(); }).getOrElse(0); // 0
```

### getOrCompute - Compute Default Lazily

```typescript
import { attempt, getOrCompute } from "@deessejs/core";

const result = attempt(() => { throw new Error(); })
  .getOrCompute(() => 42);
```

### match - Pattern Matching

```typescript
import { attempt, match } from "@deessejs/core";

const result = attempt(() => 42);
const message = match(
  result,
  (value) => `Success: ${value}`,
  (error) => `Failed: ${error.message}`
);
```

### tap - Side Effects

```typescript
import { attempt, tap } from "@deessejs/core";

attempt(() => 42).tap(value => console.log(value)); // Logs: 42
```

### tapErr - Side Effects on Error

```typescript
import { attempt, tapErr } from "@deessejs/core";

attempt(() => { throw new Error("fail"); })
  .tapErr(error => console.error(error));
```

## Type Guards

```typescript
import { attempt, isOk, isErr } from "@deessejs/core";

const result = attempt(() => 42);

if (isOk(result)) {
  console.log(result.value); // 42
}

if (isErr(result)) {
  console.log(result.error);
}
```

### isErrTryWithError - Check for Structured Error

Use with the Error system for rich error handling:

```typescript
import { attempt, isErrTryWithError, error } from "@deessejs/core";

const ValidationError = error({
  name: "ValidationError",
  args: {} as { field: string }
});

// Try with Error type
const t: Try<string, Error> = attempt(() => {
  throw ValidationError({ field: "email" });
});

if (isErrTryWithError(t)) {
  // TypeScript knows t.error is Error
  console.log(t.error.name); // "ValidationError"
  console.log(t.error.args.field); // "email"
}
```

## API Reference

| Function | Description |
|----------|-------------|
| `attempt(fn)` | Wrap a sync function |
| `attemptAsync(fn)` | Wrap an async function |
| `isOk(t)` | Type guard for success |
| `isErr(t)` | Type guard for failure |
| `map(t, fn)` | Transform the value |
| `flatMap(t, fn)` | Chain operations |
| `getOrElse(t, default)` | Get value or default |
| `getOrCompute(t, fn)` | Compute default lazily |
| `match(t, ok, err)` | Pattern matching |
| `tap(t, fn)` | Side effect on value |
| `tapErr(t, fn)` | Side effect on error |

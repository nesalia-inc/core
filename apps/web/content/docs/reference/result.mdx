...
```typescript
import { ok, err, isOk, Result } from '@deessejs/core';

function parseUser(data: string): Result<User, Error> {
  try {
    return ok(JSON.parse(data));
  } catch (e) {
    return err(new Error('Failed to parse user'));
  }
}
...
- **`result.error`** - Access the error (for rich errors with notes and cause, see [Error](./error.mdx))

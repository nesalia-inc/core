...
- **`none()`** — Represents absence (singleton)
...
Converts to `Result<T, E>`. Some becomes Ok, None becomes Err using the provided error.
...
Maps over items, returning None if any result is None.
...
- [Conversions](../conversions.mdx) - Convert between Maybe and Result

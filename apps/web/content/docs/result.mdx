...
### isErrWithError - Check for Structured Error

Use with the Error system for rich error handling:

```typescript
import { ok, err, isErrWithError, error } from "@deessejs/core";
...
const result: Result<string, Error> = err(ValidationError({ field: "email" }));
...
if (isErrWithError(result)) {
  ...
  console.log(result.error.notes); // []
}
```
